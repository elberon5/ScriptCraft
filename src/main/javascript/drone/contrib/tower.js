...
Drone.extend('tower', function(floors, side) {
  var floorHeight = 4, brick = 98, window = 102;
  ...
  // windows
  this.move('tower').up(2).fwd(halfway-1).box(window)
    .fwd(halfway).right(halfway-1).box(window)
    .right(halfway).back(halfway-1).box(window)
    .back(halfway).left(halfway-1).box(window);
  ...
  for(x = 1;x < floors; x++)  {
    ...
    this.move('tower').right(1).up(5 * (x-1)).up(1).fwd(side - 2) // move inside tower
      .box(ladder, 1,(floorHeight+ 1),1);
    // add a torch on each floor
    this.move('tower').up(5 * x).fwd(side-2).right(2).up(3)
      .box(torch);
  // windows
    this.move('tower').up(5 * x).up(2).fwd(halfway-1).box(window)
      .fwd(halfway).right(halfway-1).box(window)
      .right(halfway).back(halfway-1).box(window)
      .back(halfway).left(halfway-1).box(window);
  }
